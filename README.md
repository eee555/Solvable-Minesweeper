...
### 通过网盘安装(推荐)
在下面的[下载链接](#下载链接)中找到最新的版本，然后下载，解压，直接运行`main.exe`文件（如果警告请点击“仍然运行”），开箱即用。通过此方法安装的软件，是“官方”的软件，能够对录像文件进行官方的签名（签名功能打包在“metaminesweeper_checksum.pyd”中，占比很小，且是闭源的）。

### 通过Github Actions安装(最安全)
请注意：通过此方法安装的软件，不能对录像文件进行正确的签名。即，自行打包的软件，其生成的录像文件，无法通过“通过网盘安装”的软件的校验。其余功能均相同。  
...
### 从源码安装(不推荐)
请注意：通过此方法安装的软件，同样不能对录像文件进行正确的签名。即，自行打包的软件，其生成的录像文件，无法通过“通过网盘安装”的软件的校验。其余功能均相同。  
在编译之前，请确保自己拥有：
*   Python >=3.7
